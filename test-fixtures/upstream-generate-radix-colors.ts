 ...
 *
 *   1. the `"use client"` directive is removed (this is not a React tree)
 *   2. this `@ts-nocheck` header
 *
 * (2) exists because the upstream file predates `strict` and reads
 * `colorjs.io`'s nullable `oklch` components without narrowing. Editing the
 * body to satisfy the compiler would make it no longer upstream's algorithm,
 * which is the ONLY thing this file is here to be — the reference that
 * `color/radix.ts` is proven byte-identical against. Suppressing the check on
 * the copy is the honest trade; the port itself is fully typed.
 */
...
export const generateRadixColors = ({
	...
	return {
		// DEBUG (added for the conformance test — see radix.test.ts):
		__accentCoords: accentScaleColors.map((c) => [...c.coords]),
