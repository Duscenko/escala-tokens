...
export function ScaleRow({ scale, baseIndex = 6, showNumbers = true }: { scale: Record<number, string>; baseIndex?: number; showNumbers?: boolean }) {
...
export function LinkToggle({ active, onClick }: { active: boolean; onClick: () => void }) {
  return (
    <button
      type="button"
      onClick={onClick}
      ...
      className={`w-8 h-8 rounded-full flex items-center justify-center border transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#0088FF] ${
        active
          ? 'bg-[#0088FF]/10 border-[#0088FF] text-[#0088FF]'
          : 'bg-surface border-line-strong text-fg-faint hover:text-fg hover:border-fg-faint'
      }`}
    >
