...
export type Role = {
  ...
  scale: RoleScale
  tone: number
  // Optional explicit dark-mode override. ABSENT is the default and correct
  // choice for almost every role: `recDarkTone` reads it as IDENTITY (same
  // step, different ramp — the dark ramp already carries the right lightness
  // for that step). Only set `darkTone`/`darkScale` for a role that
  // deliberately sits on the OPPOSITE polarity in dark mode (a scrim that
  // must stay dark, text that must stay readable on a solid fill regardless
  // of theme, a border that switches ramps entirely) — see `recDarkTone`'s
  // `inverts` list. A hardcoded `darkTone` on a role that ISN'T one of those
  // is very likely a bug, not a design choice: that's exactly what silently
  // broke `background-primary` (page background) for months — a leftover
  // Tailwind-scale-style inversion (25↔950) baked in before the per-appearance
...
// Background (surfaces + solid fills) and Border. Every role resolves its
// dark tone as IDENTITY by default (same step, dark ramp — see `recDarkTone`);
// only the handful that deliberately sit on the opposite polarity pin an
// explicit `darkTone`/`darkScale`. The export emits whichever tone that
// resolves to as a DTCG reference verbatim.
...
export function recDarkTone(role: Role): number {
  ...
  const inverts =
    ...
    // A modal scrim reads its light tone (12, near-black) as a fixed "veil"
    // colour, not a step position — identity would flip it to near-white.
    role.key === 'background-overlay'
